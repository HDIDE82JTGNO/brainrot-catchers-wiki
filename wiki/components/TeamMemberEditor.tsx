...
export function TeamMemberEditor({ member, moves, items, onSave, onCancel }: TeamMemberEditorProps) {
  ...
  const [heldItem, setHeldItem] = useState<string | undefined>(member.heldItem);
  const [moveSearch, setMoveSearch] = useState('');

  // Filter out MoveLearners category items (can't be held)
  const holdableItems = useMemo(() => {
    return items.filter(item => item.Category !== 'MoveLearners');
  }, [items]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <AnimatedDiv
        style={fadeIn}
        className="bg-white rounded-2xl border-2 border-slate-200 shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto"
      >
        <div className="sticky top-0 bg-white border-b-2 border-slate-200 p-4 flex items-center justify-between z-10">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 border-2 border-slate-300 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
            />
          </div>
              ...
              {(Object.keys(ivs) as Array<keyof StatBlock>).map(stat => (
                <div key={stat}>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border-2 border-slate-300 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                  />
                </div>
              ))}
            </div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-900 mb-2">
              ...
              <span className={`ml-2 ${totalEVs > 510 ? 'text-red-600' : 'text-slate-600'}`}>
                ({totalEVs}/510)
              </span>
            </label>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 border-2 border-slate-300 rounded-lg bg-white text-slate-900 mb-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
            />
            <div className="max-h-48 overflow-y-auto border-2 border-slate-200 rounded-lg p-2">
          ...
          <div>
            ...
            <select
              value={heldItem || ''}
              onChange={(e) => setHeldItem(e.target.value || undefined)}
              className="w-full px-4 py-2 border-2 border-slate-300 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
            >
              <option value="">None</option>
              {holdableItems.map(item => (
            ...
            {heldItem && (
              <div className="mt-2 flex items-center gap-2 p-2 bg-slate-50 rounded-lg">
                {(() => {
                  const item = holdableItems.find(i => i.Name === heldItem);
                  return item ? (
                    <>
                      <ItemImage item={item} moves={moves} size={32} />
          ...
          <div className="flex gap-4 pt-4 border-t-2 border-slate-200">
            <button
              onClick={onCancel}
              className="flex-1 px-4 py-2 border-2 border-slate-300 rounded-lg hover:bg-slate-50 transition-colors font-medium text-slate-700"
            >
              Cancel
            </button>
            <button
