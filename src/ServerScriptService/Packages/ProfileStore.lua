...
local AUTO_SAVE_PERIOD = 60 -- (Seconds) Time between when changes to a profile are saved to the DataStore
...
local function SaveProfileAsync(profile, is_ending_session, is_overwriting, last_save_reason)
	...
	while repeat_save_flag == true do
		...
		if loaded_data ~= nil and key_info ~= nil then
			-- Clear one-save override if present (consume the flag exactly once)
			if data and data._AllowOneSave == true then
				data._AllowOneSave = nil
			end

			print(`[SaveProfileAsync]: Successfully saved profile {profile.Key}`)
			--Notify client to give them the save animation
			local Players = game:GetService("Players")
			local player = Players:GetPlayerByUserId(profile.UserIds[1])

			if player then
				print(`[SaveProfileAsync]: Notifying client {profile.Key}`)
				game.ReplicatedStorage.Events.Communicate:FireClient(player, "SaveSuccess")
			else
				--print(`[SaveProfileAsync]: Failed to find player.`)
				--Im 99% sure this means they've left the game and autosave has called for this save to happen, if it was a manual save we have a big issue
			end
...
RunService.Heartbeat:Connect(function()
	...
	if auto_save_list_length > 0 then
		...
		while os_clock - LastAutoSave > auto_save_index_speed do
            ...
            -- Perform save call (respect game-level gating):
            if profile ~= nil then
                local data = profile.Data or {}
                local autosaveEnabled = (data.Settings and data.Settings.AutoSave) == true
                local inBattle = data.InBattle == true
